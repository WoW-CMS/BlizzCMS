      ...
      <div class="uk-container">
        ...
        <div class="uk-card uk-card-default">
          <div class="uk-card-body">
            <?= form_open('', 'id="updatefaqForm" onsubmit="UpdateFaqForm(event)"'); ?>
            <div class="uk-margin-small">
              <label class="uk-form-label uk-text-uppercase"><?= $this->lang->line('placeholder_faq_title'); ?></label>
              <div class="uk-form-controls">
                <div class="uk-inline uk-width-1-1">
                  <span class="uk-form-icon uk-form-icon-flip" uk-icon="icon: pencil"></span>
                  <input class="uk-input" type="text" id="faq_title" value="<?= $this->admin_model->getFaqSpecifyName($idlink); ?>" placeholder="<?= $this->lang->line('placeholder_faq_title'); ?>" required>
                </div>
              </div>
            </div>
            <div class="uk-margin-small">
            ...
            <div class="uk-margin-small">
              ...
              <div class="uk-form-controls">
                <textarea class="uk-textarea tinyeditor" id="faq_description" rows="12"><?= $this->admin_model->getFaqSpecifyDesc($idlink); ?></textarea>
              </div>
            </div>
            <div class="uk-margin-small">
              <button class="uk-button uk-button-primary uk-width-1-1" type="submit" id="button_upfaq"><i class="fas fa-sync-alt"></i> <?= $this->lang->line('button_save'); ?></button>
            </div>
            <?= form_close(); ?>
          </div>
        </div>
    ...
    <script>
      function UpdateFaqForm(e) {
        ...
        $.ajax({
          url:"<?= base_url($lang.'/admin/faq/update'); ?>",
          method:"POST",
          data:{id, title, type, description},
          dataType:"text",
          beforeSend: function(){
            $.amaran({
              'theme': 'awesome info',
          ...
          success:function(response){
            ...
            window.location.replace("<?= base_url('admin/faq/edit/'.$idlink); ?>");
          }
        });
      }
    </script>
