...
class BS_Cart extends CI_Cart
{
    ...
    /**
     * These are the regular expression rules that we use to validate the product ID and product name
     * alpha-numeric with spaces and a limited set of characters: _ - . : ' [ ]
     *
     * @var string
     */
    ...
    public function __construct($params = [])
    {
        ...
        if ($this->_cart_contents === null)
        {
            ...
            $this->_cart_contents = ['total_items' => 0, 'total_dp' => 0, 'total_vp' => 0];
        }
    ...
    protected function _insert($items = [])
    {
        ...
        // Does the $items array contain an id, quantity, name, dp and vp?  These are required
        if (! isset($items['id'], $items['qty'], $items['name'], $items['dp'], $items['vp']))
        {
            log_message('error', 'The cart array must contain a product ID, quantity, name, dp and vp.');
            return false;
        }
        ...
        $items['dp'] = (int) $items['dp'];
        $items['vp'] = (int) $items['vp'];
    ...
    protected function _save_cart()
    {
        ...
        $this->_cart_contents['total_items'] = $this->_cart_contents['total_dp'] = $this->_cart_contents['total_vp'] = 0;
        foreach ($this->_cart_contents as $key => $val)
        {
            // We make sure the array contains the proper indexes
            if (! is_array($val) OR ! isset($val['dp'], $val['qty']) OR ! isset($val['vp'], $val['qty']))
            {
                continue;
            }
    ...
    {
        return (in_array($row_id, ['total_items', 'total_dp', 'total_vp'], true) OR ! isset($this->_cart_contents[$row_id]))
            ? false
